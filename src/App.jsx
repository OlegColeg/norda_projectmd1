...
import { Star, Moon, Sparkles, Mail, Phone, MapPin, Instagram, Plus, Trash2, Edit2, Save, X, Calendar, Clock, Type, Palette, RotateCcw } from 'lucide-react';
...
const initNotifications = async () => {
  try {
    // Cere permisiuni pentru notificări pe Android
    if ('permissions' in navigator) {
      const permission = await navigator.permissions.query({ name: 'notifications' });
      if (permission.state === 'denied') return;
    }

    // Cere permisiuni pe web browsers
    if ('Notification' in window && Notification.permission === 'default') {
      Notification.requestPermission();
    }

    // Inițializează LocalNotifications pentru Android
    ...
    // Registrează service worker pentru web notifications
...
// Trimite notificare pe Android via Capacitor
const sendAndroidNotification = async (title, body) => {
  try {
    ...
    console.log('Android notification error:', error);
  }
};

// Funcție universală de notificare
const triggerOrderNotification = (orderNumber, clientName, productName, price) => {
  const title = `📦 Comandă #${orderNumber}`;
  const body = `De la: ${clientName}\nProdus: ${productName}\nPreț: ${price} MDL`;
...
export default function NordaStarMaps() {
  ...
  // Reference pentru tracking comenzi noi (pentru notificări)
  ...
  // Auto-refresh comenzi fiecare 5 secunde când ești pe pagina admin-orders
  useEffect(() => {
    if (currentPage === 'admin-orders' && isAdmin) {
      const interval = setInterval(() => {
        loadOrders();
      }, 5000); // Refresh la fiecare 5 secunde
      return () => clearInterval(interval);
    }
  }, [currentPage, isAdmin]);

  // Ascultă pentru comenzi noi și trimite notificări
  useEffect(() => {
    if (!isAdmin) return;

    // Inițializează notificări doar dacă e admin
    initNotifications();

    // Setup Firestore listener pentru comenzi noi
    const unsubscribe = onSnapshot(collection(db, "orders"), (querySnapshot) => {
      ...
      // Comparează comenzi anterioare cu cele noi
      if (ordersRef.current.length > 0) {
        const newOrders = allOrders.filter(newOrder =>
          !ordersRef.current.some(oldOrder => oldOrder.id === newOrder.id)
        );

        // Trimite notificări pentru comenzile noi
        newOrders.forEach(order => {
          ...
          console.log('Notificare trimisă pentru comandă:', order.orderNumber);
        });
      }

      // Actualizează referința cu comenzile actuale
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setCurrentPage('admin-orders')} className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-gray-900 font-bold text-sm rounded-lg transition">Comenzi</button>
              <button onClick={async () => { try { await signOut(auth); setIsAdmin(false); setCurrentPage('home'); } catch(err) { console.error('Logout error', err); setIsAdmin(false); setCurrentPage('home'); } }} className="text-red-400 hover:text-red-300 transition text-sm font-semibold">Deconectare</button>
            </div>
          </div>
      ...
      <main>
        ...
        {currentPage === 'admin-orders' && isAdmin && (
          <div className="min-h-screen bg-gradient-to-b from-gray-900 to-blue-900 pt-32 pb-20">
            <div className="max-w-7xl mx-auto px-4">
              <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-white">📋 Comenzi</h1>
                <div className="flex items-center gap-3">
                  <button onClick={() => setCurrentPage('admin')} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm rounded-lg transition">Produse</button>
                  <button onClick={loadOrders} className="p-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition" title="Refresh manual">
                    <RotateCcw size={18} />
                  </button>
                  <button onClick={() => {setIsAdmin(false); setCurrentPage('home');}} className="text-red-400 hover:text-red-300 transition text-sm font-semibold">Deconectare</button>
                </div>
              </div>
              ...
              <div className="flex gap-4 mb-8 items-center">
                ...
                <div className="ml-auto flex items-center gap-2 bg-green-500/20 border border-green-500/50 px-4 py-2 rounded-lg">
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                  <span className="text-green-400 text-sm font-semibold">Auto-refresh activ</span>
                </div>
              </div>
